  ...
  <div class="slider-wrapper theme-default">
    <div id="slider" class="nivoSlider">
        ...
        while ( $query->have_posts() ) : $query->the_post();
          $i++;
          $recipe_lite_slideno[] = $i;
          $recipe_lite_slidetitle[] = get_the_title();
		  $recipe_lite_slidedesc[] = get_the_excerpt();
          $recipe_lite_slidelink[] = get_permalink();
          ?>
        ...
        <?php
        $k = 0;
        foreach( $recipe_lite_slideno as $recipe_lite_sln ){ ?>
    <div id="slidecaption<?php echo esc_attr( $recipe_lite_sln ); ?>" class="nivo-html-caption">
      <div class="slide_info">
        <h2><?php echo esc_html($recipe_lite_slidetitle[$k] ); ?></h2>
        <p><?php echo esc_html($recipe_lite_slidedesc[$k] ); ?></p>
        <div class="clear"></div>
        <a class="slide_more" href="<?php echo esc_url($recipe_lite_slidelink[$k] ); ?>">
          <?php esc_html_e('Read More', 'recipe-lite');?>
          </a>
      </div>
    </div>
 	<?php $k++;
...
?>
 <section id="sec2fourbox">
    ...
    <div class="specialty-boxes-outer"><div class="specialty-boxes specialty-boxes-2column">

      <?php for($z=1; $z<5; $z++) { ?>
      <?php if( get_theme_mod('sec-column'.$z,false)) { ?>
      <?php $seccolbox = new WP_query('page_id='.get_theme_mod('sec-column'.$z,true)); ?>
      <?php while( $seccolbox->have_posts() ) : $seccolbox->the_post(); ?>
		<div class="specialty-box">
			<div class="specialty-box-inner">
        ...
        <?php
                    if ( have_posts() ) :
						...
						the_posts_pagination( array(
							'mid_size' => 2,
							'prev_text' => esc_html__( 'Back', 'recipe-lite' ),
							'next_text' => esc_html__( 'Next', 'recipe-lite' ),
						) );
