...
function ckReadDndImages(holder, files) {
	...
	if (!!window.FormData) {
		...
		var myurl = 'index.php?option=com_maximenuck&task=browse.ajaxAddPicture&' + cktoken + '=1';
		$ck.ajax({
			type: "POST",
			url: myurl,
			// async: false,
...
function ckCreateFolder(btn, path) {
	...
	var myurl = MAXIMENUCK.BASE_URL + "&task=browse.ajaxCreateFolder&" + MAXIMENUCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		data: {
