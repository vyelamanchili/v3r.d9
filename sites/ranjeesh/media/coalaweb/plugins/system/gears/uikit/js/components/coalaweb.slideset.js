...
(function(addon) {
    ...
    UI.component('slideset', {
        ...
        init: function() {
            ...
            this.list      = this.element.find('.cw-slideset');
            ...
            $this.list.addClass('cw-grid-width-1-'+$this.options.default);
            ...
            this.on("click.uk.slideset", '[data-cw-slideset-item]', function(e) {
                ...
                var set = UI.$(this).attr('data-cw-slideset-item');
            ...
            this.controls.on('click.uk.slideset', '[data-cw-filter]', function(e) {

                var ele = UI.$(this);

                if (ele.parent().hasClass('cw-slideset')) {
                    return;
                }

                e.preventDefault();

                if ($this.animating || $this.currentFilter == ele.attr('data-cw-filter')) {
                    return;
                }

                $this.updateFilter(ele.attr('data-cw-filter'));
            ...
            UI.domObserve(this.list, function(e) {
                if ($this.list.children(':visible:not(.cw-active)').length) {
        ...
        update: function(animate, force) {
            ...
            if (this.nav.length && this.nav.empty()) {
                ...
                for (i=0;i<this.sets.length;i++) {
                    this.nav.append('<li data-cw-slideset-item="'+i+'"><a></a></li>');
                }

                this.nav[this.nav.children().length==1 ? 'addClass':'removeClass']('cw-invisible');
            }
        ...
        updateFilter: function(currentfilter) {
            ...
            this.controls.find('[data-cw-filter]').each(function(){

                filter = UI.$(this);

                if (!filter.parent().hasClass('cw-slideset')) {

                    if (filter.attr('data-cw-filter') == $this.currentFilter) {
                        filter.addClass('cw-active');
                    } else {
                        filter.removeClass('cw-active');
                    }
                }
            });
        },
        ...
        getVisibleOnCurrenBreakpoint: function() {
            ...
            var breakpoint  = null,
                ...
                ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                    ...
                    tmp.attr('class', 'cw-grid-width-'+bp+'-1-2').width();
        ...
        getItems: function() {
            ...
            if (this.currentFilter) {
                ...
                this.children.each(function(index){
                    ...
                    var ele = UI.$(this), f = ele.attr('data-cw-filter'), infilter = filter.length ? false : true;
        ...
        show: function(setIndex, noanimate, dir) {
            ...
            if (this.nav.length) {
                this.nav.children().removeClass('cw-active').eq(setIndex).addClass('cw-active');
            }
            ...
            animation.apply($this, [current, next, dir]).then(function(){
                ...
                $this.children.hide().removeClass('cw-active');
                next.addClass('cw-active').css({'display': '', 'opacity':''});
    ...
    function coreAnimation(cls, current, next, dir) {
        ...
        release = function() {
            ...
            if (current && current.length) {
                current.hide().removeClass(clsOut+' cw-animation-reverse').css({'opacity':'', 'animation-delay': '', 'animation':''});
            }
            ...
            for (i=0;i<current.length;i++) {
                ...
                (function (index, ele){
                    ...
                    setTimeout(function(){
                        ...
                        }).addClass(clsOut+' cw-animation-reverse');
