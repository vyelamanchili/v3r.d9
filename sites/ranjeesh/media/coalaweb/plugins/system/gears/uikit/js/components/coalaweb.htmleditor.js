...
(function(addon) {
    ...
    UI.component('htmleditor', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('textarea[data-cw-htmleditor]', context).each(function() {

                    var editor = UI.$(this);

                    if (!editor.data('htmleditor')) {
                        UI.htmleditor(editor, UI.Utils.options(editor.attr('data-cw-htmleditor')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.content    = this.htmleditor.find('.cw-htmleditor-content');
            this.toolbar    = this.htmleditor.find('.cw-htmleditor-toolbar');
            this.preview    = this.htmleditor.find('.cw-htmleditor-preview').children().eq(0);
            this.code       = this.htmleditor.find('.cw-htmleditor-code');
            ...
            if (this.options.iframe) {
                ...
                this.iframe = UI.$('<iframe class="cw-htmleditor-iframe" frameborder="0" scrolling="auto" height="100" width="100%"></iframe>');
            ...
            this.htmleditor.on('click', '.cw-htmleditor-button-code, .cw-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.htmleditor.attr('data-mode') == 'tab') {

                    $this.htmleditor.find('.cw-htmleditor-button-code, .cw-htmleditor-button-preview').removeClass('cw-active').filter(this).addClass('cw-active');

                    $this.activetab = UI.$(this).hasClass('cw-htmleditor-button-code') ? 'code' : 'preview';
            ...
            this.element.attr('data-cw-check-display', 1).on('display.uk.check', function(e) {
        ...
        _buildtoolbar: function() {
            ...
            this.options.toolbar.forEach(function(button) {
                ...
                bar.push('<li><a data-htmleditor-button="'+button+'" title="'+title+'" data-cw-tooltip>'+$this.buttons[button].label+'</a></li>');
            });
        ...
        fit: function() {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.cw-htmleditor-button-code, .cw-htmleditor-button-preview').removeClass('cw-active')
                    .filter(this.activetab == 'code' ? '.cw-htmleditor-button-code' : '.cw-htmleditor-button-preview')
                    .addClass('cw-active');
            }
    ...
    UI.components.htmleditor.template = [
        '<div class="cw-htmleditor cw-clearfix" data-mode="split">',
            '<div class="cw-htmleditor-navbar">',
                '<ul class="cw-htmleditor-navbar-nav cw-htmleditor-toolbar"></ul>',
                '<div class="cw-htmleditor-navbar-flip">',
                    '<ul class="cw-htmleditor-navbar-nav">',
                        '<li class="cw-htmleditor-button-code"><a>{:lblCodeview}</a></li>',
                        '<li class="cw-htmleditor-button-preview"><a>{:lblPreview}</a></li>',
                        '<li><a data-htmleditor-button="fullscreen"><i class="cw-icon-expand"></i></a></li>',
                    '</ul>',
                '</div>',
            '</div>',
            '<div class="cw-htmleditor-content">',
                '<div class="cw-htmleditor-code"></div>',
                '<div class="cw-htmleditor-preview"><div></div></div>',
            '</div>',
        '</div>'
    ].join('');
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.addButtons({
                ...
                fullscreen: {
                    title  : 'Fullscreen',
                    label  : '<i class="cw-icon-expand"></i>'
                },
                bold : {
                    title  : 'Bold',
                    label  : '<i class="cw-icon-bold"></i>'
                },
                italic : {
                    title  : 'Italic',
                    label  : '<i class="cw-icon-italic"></i>'
                },
                strike : {
                    title  : 'Strikethrough',
                    label  : '<i class="cw-icon-strikethrough"></i>'
                },
                blockquote : {
                    title  : 'Blockquote',
                    label  : '<i class="cw-icon-quote-right"></i>'
                },
                link : {
                    title  : 'Link',
                    label  : '<i class="cw-icon-link"></i>'
                },
                image : {
                    title  : 'Image',
                    label  : '<i class="cw-icon-picture-o"></i>'
                },
                listUl : {
                    title  : 'Unordered List',
                    label  : '<i class="cw-icon-list-ul"></i>'
                },
                listOl : {
                    title  : 'Ordered List',
                    label  : '<i class="cw-icon-list-ol"></i>'
                }
            ...
            editor.htmleditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                ...
                editor.htmleditor.toggleClass('cw-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.htmleditor.hasClass('cw-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            UI.$.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.htmleditor.find('.cw-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.htmleditor.find('.cw-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }
