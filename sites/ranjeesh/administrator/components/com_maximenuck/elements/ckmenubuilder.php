...
class JFormFieldCkmenubuilder extends JFormField {
	...
	protected function getInput() {
		...
		$html .= '<div class="' . (version_compare(JVERSION, '4') >= 0 ?  'btn btn-outline-secondary'  :'ckbutton' ) . '" onclick="CKBox.open({url: \'index.php?option=com_maximenuck&view=menubuilders&tmpl=component&layout=modal\', style: {padding: \'0px\'}})"><i class="fas fa-mouse-pointer "></i> ' . JText::_('CK_SELECT') . '</div>';
		$html .= '<div class="' . (version_compare(JVERSION, '4') >= 0 ?  'btn btn-outline-secondary'  :'ckbutton' ) . '" onclick="CKBox.open({url: \'index.php?option=com_maximenuck&view=menubuilder&layout=modal&tmpl=component&id=\'+jQuery(\'#' . $this->id . '\').val()+\'\'})"><i class="fas fa-edit"></i> ' . JText::_('CK_EDIT') . '</div>';
		$html .= '<div class="' . (version_compare(JVERSION, '4') >= 0 ?  'btn btn-outline-secondary'  :'ckbutton' ) . ' cktip" onclick="jQuery(\'#' . $this->id . '\').val(\'\');jQuery(\'#' . $this->id . 'name\').val(\'\');" title="' . JText::_('CK_REMOVE') . '"><i class="fas fa-times"></i></div>';
		$html .= '</div>';
